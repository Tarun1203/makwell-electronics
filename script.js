// script.js — Products page logic (MakWell)
(() => {
  ...
  const applyFilters = () => {
    ...
    if (q) list = list.filter(p => `${p.name} ${p.title || ''} ${p.description || ''} ${(p.tags || []).join(' ')} ${(p.specs || []).join(' ')}`.toLowerCase().includes(q))
    if (state.category !== 'all') list = list.filter(p => String(p.category).toLowerCase() === state.category.toLowerCase())
  ...
  const render = () => {
    ...
    for (const p of pageItems) {
      ...
      const imgSrc = p.image || 'data:image/svg+xml,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300"><rect width="100%" height="100%" fill="white"/><text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" font-size="20" fill="black">No image</text></svg>`)
      ...
      card.innerHTML = `
        <div class="media"><img alt="${(p.name || p.title || 'Product').replace(/"/g, '&quot;')}" loading="lazy" src="${imgSrc}"></div>
        <div class="body">
  ...
  const renderCart = () => {
    ...
    for (const [id, qty] of Object.entries(state.cart)) {
      ...
      row.innerHTML = `
        <img src="${p.image || ''}" alt="${p.name || p.title || ''}" style="width:48px;height:48px;object-fit:cover;border-radius:8px">
        <div>
      ...
      <div style="position:fixed;inset:0;background:rgba(0,0,0,.4);display:grid;place-items:center;padding:20px;z-index:50">
        <article class="card" style="max-width:720px;width:100%;overflow:hidden">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0;max-height:80vh">
            <div class="media" style="height:100%"><img src="${p.image || ''}" alt="${p.name || p.title}"></div>
            <div class="body" style="overflow:auto">
              <h2 style="margin:0 0 6px">${p.name || p.title}</h2>
              <div class="price">${priceHtml}</div>
              <p class="muted">${p.description || ''}</p>
              <div style="display:flex;gap:6px;flex-wrap:wrap;margin:8px 0">
                ${os}${series}${badgesHtml}
              </div>
  ...
  async function loadProducts() {
    const tryPaths = ['products.json', 'data/products.json', '/perp/products.json']
    let json = null
    ...
    if (!json) { toast('Could not find products.json'); json = [] }

    state.products = (json || []).map((p, i) => ({
      id: p.id ?? i + 1,
      name: p.name ?? p.title ?? `Product #${i + 1}`, // accept title as name
      title: p.title ?? p.name ?? '',
      price: (p.price ?? null),                        // null = contact
      image: p.image ?? '',
  ...
  const clear = el('#clearFilters'); if (clear) clear.onclick = () => { state.query=''; state.category='all'; state.sort='popular'; if(q) q.value=''; if(cat) cat.value='all'; if(sort) sort.value='popular'; applyFilters() }
  ...
  const co = el('#checkout'); if (co) co.onclick = () => {
    ...
    alert('Checkout payload:\\n' + JSON.stringify(payload, null, 2)) // replace with Razorpay/Stripe later
  }
  window.addEventListener('keydown', (e) => { if (e.key === '/' && document.activeElement.tagName !== 'INPUT') { e.preventDefault(); q && q.focus() } })
