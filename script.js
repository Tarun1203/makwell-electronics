...
(() => {
  ...
  const toImageUrl = (path) => {
    ...
    return file ? (RAW_BASE + file) : PLACEHOLDER;
  };
  ...
  const toast = (msg) => { const t = el('#toast'); if (!t) return; t.textContent = msg; t.style.opacity = '1'; clearTimeout(t._t); t._t = setTimeout(() => t.style.opacity = '0', 1500); };
  ...
  const applyFilters = () => {
    ...
    if (q) {
      list = list.filter(p =>
        `${p.name} ${p.title || ''} ${p.description || ''} ${(p.tags || []).join(' ')} ${(p.specs || []).join(' ')}`
  ...
  const renderCart = () => {
    ...
    for (const [id, qty] of Object.entries(state.cart)) {
      ...
      const img = row.querySelector('img'); img.onerror = () => { img.src = PLACEHOLDER; };
  ...
  const openDetails = (id) => {
    ...
    const img = wrap.querySelector('img'); img.onerror = () => { img.src = PLACEHOLDER; };
  ...
  async function loadProducts() {
    // products.json must be in your repo root
    let json = null;
    try {
    ...
    state.products = (json || []).map((p, i) => ({
      ...
      price: (p.price ?? null),
      image: p.image ?? '', // kept as-is; resolved via toImageUrl()
